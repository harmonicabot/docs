...
Prompt rules can also be added through the **Review** tab on the session results page (Pro). When you apply a finding from that tab, it appends a rule to this prompt. See [Reviewing facilitator behavior](/guides/hosts/session-results#reviewing-facilitator-behavior).
...
Every change to the facilitation prompt and summary prompt is tracked. Open the **History** button next to either prompt in [Session Design](/guides/hosts/session-design) to see a timestamped log of edits — including who made each change and what triggered it (a manual edit, a template switch, a brief-field cascade, or a Review-tab suggestion you applied).

You can restore any previous version directly from the history view. The restored text is staged for you to review before saving, so you won't accidentally overwrite your current prompt without seeing the diff.
...
Closing a session stops new participant activity while keeping everything the session
produced. Participant threads, messages, summaries and other session data are preserved,
so closing is never destructive and is safe to undo.

Reopening a closed session makes it active again, so participants can join or continue
where they left off.

Both actions are available on the session itself, through the
[API](/api-reference/introduction) as `POST /sessions/{id}/close` and
`POST /sessions/{id}/reopen`, and through the
[MCP server](/api-reference/mcp-server) as `close_session` and `reopen_session`.
Closing requires editor access or higher.
