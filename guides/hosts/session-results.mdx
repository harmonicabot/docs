...
## Knowing when a session has covered its ground

As participants contribute, Harmonica tracks two things about the conversation and shows them on the results overview: **coverage**, how much of the session's ground has been explored, and **saturation**, whether new responses are still adding signal or mostly repeating what has already been said. Together they give you a read on whether a session is still productive or has effectively settled, so you are not guessing when to close it or move on.

### Inviting a reflection round

Once a session settles, you can invite participants back for a **reflection round**. Instead of asking the original questions again, the facilitator presents the group's own map, the themes and tensions that emerged, and asks people to react to it. A reflection round either re-settles the map into worked-through consensus, now that the group has seen and confirmed it, or breaks it open with new ground the earlier rounds missed.

When a session has settled, a reflection-round action appears on the results overview. Arming it gives you an invite message and a join link to share, and you own how you distribute it, the same way you share any session. You can end the round at any time.
